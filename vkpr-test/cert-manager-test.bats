...
setup_file() {
    ...
    if [ "$VKPR_TEST_SKIP_SETUP" == "true" ]; then
        ...
        $VKPR_KUBECTL create namespace cert-manager
        $VKPR_KUBECTL create secret generic custom-ca-secret --namespace cert-manager \
        ...
        $VKPR_KUBECTL wait --all-namespaces --for=condition=ready --timeout=5m pod --all
        sleep 2
