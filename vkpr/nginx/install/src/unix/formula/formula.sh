...
runFormula() {
  ...
  VKPR_INGRESS_VALUES="$(dirname "$0")"/utils/nginx.yaml

  startInfos
  settingIngress
  [ $DRY_RUN = false ] && registerHelmRepository ingress-nginx https://kubernetes.github.io/ingress-nginx
  installApplication "nginx" "ingress-nginx/ingress-nginx" "$VKPR_ENV_NGINX_NAMESPACE" "$VKPR_INGRESS_NGINX_VERSION" "$VKPR_INGRESS_VALUES" "$HELM_ARGS"
}

startInfos() {
  bold "=============================="
  boldInfo "VKPR Nginx Install Routine"
  bold "=============================="
}

formulaInputs() {
  # App values
  checkGlobalConfig "$VKPR_ENV_GLOBAL_NAMESPACE" "$VKPR_ENV_GLOBAL_NAMESPACE" "nginx.namespace" "NGINX_NAMESPACE"
  checkGlobalConfig "false" "false" "nginx.metrics" "INGRESS_METRICS"
  checkGlobalConfig "$SSL" "false" "nginx.ssl.enabled" "INGRESS_SSL"
  checkGlobalConfig "$CRT_FILE" "" "nginx.ssl.crt" "INGRESS_SSL_CERTIFICATE"
  checkGlobalConfig "$KEY_FILE" "" "nginx.ssl.key" "INGRESS_SSL_KEY"
  checkGlobalConfig "nginx-cert" "nginx-cert" "nginx.ssl.secretName" "INGRESS_SSL_SECRET"
...
validateInputs() {
  validateIngressNamespace "$VKPR_ENV_NGINX_NAMESPACE"
...
settingIngress() {
  ...
  if [[ "$VKPR_ENV_INGRESS_METRICS" == "true" ]] && [[ $(checkPodName "$VKPR_ENV_GRAFANA_NAMESPACE" "prometheus-stack-grafana") == "true" ]]; then
    ...
    YQ_VALUES="$YQ_VALUES |
      ...
      .controller.metrics.serviceMonitor.namespace = \"$VKPR_ENV_NGINX_NAMESPACE\" |
      .controller.metrics.serviceMonitor.additionalLabels.release = \"prometheus-stack\"
     "
  fi

  if [[ "$VKPR_ENV_INGRESS_SSL" == "true" ]]; then
    [[ "$VKPR_ENV_INGRESS_SSL_SECRET" == "nginx-cert" ]] && $VKPR_KUBECTL create secret tls nginx-cert -n "$VKPR_ENV_NGINX_NAMESPACE" \
      --cert="$VKPR_ENV_INGRESS_SSL_CERTIFICATE" \
      --key="$VKPR_ENV_INGRESS_SSL_KEY"
    YQ_VALUES="$YQ_VALUES |
      .controller.extraArgs.default-ssl-certificate = \"$VKPR_ENV_NGINX_NAMESPACE/$VKPR_ENV_INGRESS_SSL_SECRET\"
     "
  fi
