...
runFormula() {
  ...
  if [[ $(checkPodName $VKPR_ENV_NAMESPACE "postgres-postgresql") = "true" ]]; then
    [[ ! -z $($VKPR_KUBECTL get pod -n $VKPR_ENV_NAMESPACE | grep pgpool) ]] && PG_HOST="postgres-postgresql-pgpool"
    $VKPR_KUBECTL run init-db --rm -it --restart="Never" --namespace $VKPR_ENV_NAMESPACE --image docker.io/bitnami/postgresql-repmgr:11.14.0-debian-10-r12 --env="PGUSER=postgres" --env="PGPASSWORD=$PG_PASSWORD" --env="PGHOST=${PG_HOST}" --env="PGPORT=5432" --env="PGDATABASE=postgres" \
