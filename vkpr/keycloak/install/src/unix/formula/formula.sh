...
settingKeycloak(){
  ...
  if [[ $VKPR_ENV_GLOBAL_SECURE = true ]]; then
    YQ_VALUES="$YQ_VALUES |
      ...
      .extraEnvVars[1].name = \"KEYCLOAK_PRODUCTION\" |
  ...
  if [[ $VKPR_ENV_KEYCLOAK_METRICS == "true" ]] && [[ $(checkPodName "$VKPR_ENV_GRAFANA_NAMESPACE" "prometheus-stack-grafana") == "true" ]]; then
    ...
    YQ_VALUES="$YQ_VALUES |
      ...
      .extraEnvVars[2].name = \"URI_METRICS_ENABLED\" |
      .extraEnvVars[2].value = \"true\" |
      .extraEnvVars[3].name = \"URI_METRICS_DETAILED\" |
      .extraEnvVars[3].value = \"true\"
    "
  fi
