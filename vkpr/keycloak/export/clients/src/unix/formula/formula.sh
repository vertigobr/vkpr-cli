...
runFormula() {
  ...
  "
  $VKPR_KUBECTL cp keycloak-0:tmp/${REALM_NAME}-clientsid.json clientsid.json -n $VKPR_K8S_NAMESPACE
  echo "{}" | $VKPR_JQ --argjson groupInfo "$(<clientsid.json)" '.clients += $groupInfo' > ${REALM_NAME}-clientsid.json
  rm clientsid.json
  $VKPR_KUBECTL exec -it keycloak-0 -n $VKPR_K8S_NAMESPACE -- sh -c "rm -f /tmp/${REALM_NAME}-clientsid.json"
}
