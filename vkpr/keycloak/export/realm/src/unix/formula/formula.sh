...
runFormula() {
  ...
  "
  $VKPR_KUBECTL cp keycloak-0:tmp/${REALM_NAME}-realm.json ${REALM_NAME}-realm.json -n $VKPR_K8S_NAMESPACE
  $VKPR_KUBECTL exec -it keycloak-0 -n $VKPR_K8S_NAMESPACE -- sh -c "rm -f /tmp/${REALM_NAME}-realm.json"
}
