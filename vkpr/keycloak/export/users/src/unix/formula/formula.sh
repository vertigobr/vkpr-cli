...
runFormula() {
  ...
  "
  $VKPR_KUBECTL cp keycloak-0:tmp/${REALM_NAME}-users.json users.json -n $VKPR_K8S_NAMESPACE
  echo "{}" | $VKPR_JQ --argjson groupInfo "$(<users.json)" '.users += $groupInfo' > ${REALM_NAME}-users.json
  rm users.json
  $VKPR_KUBECTL exec -it keycloak-0 -n $VKPR_K8S_NAMESPACE -- sh -c "rm -f /tmp/${REALM_NAME}-users.json"
}
