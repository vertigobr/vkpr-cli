...
installCertManager(){
  local CERT_MANAGER_EXISTS=$($VKPR_YQ eval .global.cert-manager.enabled $VKPR_GLOBAL_CONFIG)
  if [ "$CERT_MANAGER_EXISTS" = true ]; then
    local CERT_MANAGER_PROVIDER=$($VKPR_YQ eval .global.cert-manager.provider $VKPR_GLOBAL_CONFIG)
    local CERT_MANAGER_SOLVER=$($VKPR_YQ eval .global.cert-manager.solver $VKPR_GLOBAL_CONFIG)
    [[ $CERT_MANAGER_SOLVER = "HTTP01" ]] && CERT_MANAGER_PROVIDER=""
    local AVAILABLE_CERT_MANAGER_HOSTEDZONE=""
    case $CERT_MANAGER_PROVIDER in
      aws)
        local CERT_MANAGER_ACCESS_KEY=$($VKPR_YQ eval .global.cert-manager.aws.accessKey $VKPR_GLOBAL_CONFIG)
        local CERT_MANAGER_SECRET_KEY=$($VKPR_YQ eval .global.cert-manager.aws.secretKey $VKPR_GLOBAL_CONFIG)
        local CERT_MANAGER_REGION=$($VKPR_YQ eval .global.cert-manager.aws.region $VKPR_GLOBAL_CONFIG)
        ...
        ;;
      digitalocean)
        local CERT_MANAGER_API_TOKEN=$($VKPR_YQ eval .global.cert-manager.digitalocean.apiToken $VKPR_GLOBAL_CONFIG)
...
installExternalDNS(){
  local EXTERNAL_DNS_EXISTS=$($VKPR_YQ eval .global.external-dns.enabled $VKPR_GLOBAL_CONFIG)
  if [ "$EXTERNAL_DNS_EXISTS" = true ]; then
    local EXTERNAL_DNS_PROVIDER=$($VKPR_YQ eval .global.external-dns.provider $VKPR_GLOBAL_CONFIG)
    case $EXTERNAL_DNS_PROVIDER in
      aws)
        local EXTERNAL_DNS_ACCESS_KEY=$($VKPR_YQ eval .global.external-dns.aws.accessKey $VKPR_GLOBAL_CONFIG)
        local EXTERNAL_DNS_SECRET_KEY=$($VKPR_YQ eval .global.external-dns.aws.secretKey $VKPR_GLOBAL_CONFIG)
        local EXTERNAL_DNS_REGION=$($VKPR_YQ eval .global.external-dns.aws.region $VKPR_GLOBAL_CONFIG)
        rit set credential --provider="aws" \
          --fields="accesskeyid,secretaccesskey,region" \
          --values="$EXTERNAL_DNS_ACCESS_KEY,$EXTERNAL_DNS_SECRET_KEY,$EXTERNAL_DNS_REGION"
        ;;
      digitalocean)
        local EXTERNAL_DNS_API_TOKEN=$($VKPR_YQ eval .global.external-dns.digitalocean.apiToken $VKPR_GLOBAL_CONFIG)
        rit set credential --provider="digitalocean" --fields="token" --values="$EXTERNAL_DNS_API_TOKEN"
        ;;
      powerDNS)
        local EXTERNAL_DNS_API_TOKEN=$($VKPR_YQ eval .global.external-dns.powerDNS.apiKey $VKPR_GLOBAL_CONFIG)
