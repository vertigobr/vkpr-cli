...
validateMockServerIngressClassName(){
  if [[ "$1" =~ ^([a-z]+)$ ]]; then
   return
    ...
    error "The value used for VKPR_ENV_MOCKSERVER_INGRESS_CLASS_NAME \"$1\" is invalid: VKPR_ENV_MOCKSERVER_INGRESS_CLASS_NAME must consist of lowercase alphanumeric characters, (e.g. 'mockserver', regex used for validation is ^([a-z]+)$')"
    exit
  fi
}
...
validateMockServerCertificate(){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_MOCKSERVER_CERTIFICATE \"$1\" is invalid: VKPR_ENV_MOCKSERVER_CERTIFICATE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'vkpr/mockserver/certificate.crt', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
...
validateMockServerKey(){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_MOCKSERVER_KEY \"$1\" is invalid: VKPR_ENV_MOCKSERVER_KEY must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'vkpr/mockserver/certificate.key', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
