...
validateKeycloakIngresClassName (){
  if [[ "$1" =~ ^([a-z]+)$ ]]; then
    ...
    error "The value used for VKPR_ENV_KEYCLOAK_INGRESS_CLASS_NAME \"$VKPR_ENV_KEYCLOAK_INGRESS_CLASS_NAME\" is invalid: VKPR_ENV_KEYCLOAK_INGRESS_CLASS_NAME must consist of lowercase, (e.g. 'nginx', regex used for validation is ^([a-z]+)$')"
    exit
  fi
}
...
validateKeycloakNamespace (){
  if $(validateNamespace $1); then
    ...
    error "The value used for VKPR_ENV_KEYCLOAK_NAMESPACE \"$VKPR_ENV_KEYCLOAK_NAMESPACE\" is invalid: VKPR_ENV_KEYCLOAK_NAMESPACE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'keycloak', regex used for validation is ^([A-Za-z0-9-]+)$')"
    exit
  fi
}
...
validateKeycloakCrt (){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_KEYCLOAK_CERTIFICATE \"$VKPR_ENV_KEYCLOAK_CERTIFICATE\" is invalid: VKPR_ENV_KEYCLOAK_CERTIFICATE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. '/tmp/certificate.crt', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
...
validateKeycloakKey (){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_KEYCLOAK_KEY \"$VKPR_ENV_KEYCLOAK_KEY\" is invalid: VKPR_ENV_KEYCLOAK_KEY must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. '/tmp/certificate.key', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
