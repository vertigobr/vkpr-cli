...
validateKongEnterpriseLicensePath() {
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_KONG_ENTERPRISE_LICENSE \"$VKPR_ENV_KONG_ENTERPRISE_LICENSE\" is invalid: VKPR_ENV_KONG_ENTERPRISE_LICENSE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. '/tmp/license.json', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
...
validateKongDeployment() {
  if [[ "$1" =~ ^(standard|hybrid|dbless)$ ]]; then
    ...
    error "The value used for VKPR_ENV_KONG_MODE \"$VKPR_ENV_KONG_MODE\" is invalid: VKPR_ENV_KONG_MODE must consist of standard, hybrid or dbless value"
    exit
  fi
}
