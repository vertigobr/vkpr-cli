...
validateWhoamiSslCrtPath (){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_WHOAMI_SSL_CERTIFICATE \"$1\" is invalid: VKPR_ENV_WHOAMI_SSL_CERTIFICATE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'vkpr/whoami/certificate.crt', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
...
validateWhoamiSslKeyPath (){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_WHOAMI_SSL_KEY \"$1\" is invalid: VKPR_ENV_WHOAMI_SSL_KEY must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'vkpr/whoami/certificate.key', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
