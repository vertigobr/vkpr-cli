...
validateVaultCertificate(){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_VAULT_CERTIFICATE \"$1\" is invalid: VKPR_ENV_VAULT_CERTIFICATE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'vkpr/vault/certificate.crt', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
...
validateVaultKey(){
  if $(validatePath $1); then
    ...
    error "The value used for VKPR_ENV_VAULT_KEY \"$1\" is invalid: VKPR_ENV_VAULT_KEY must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. 'vkpr/vault/certificate.key', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
    exit
  fi
}
