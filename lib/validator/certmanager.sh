...
validateCertManagerEmail() {
  if $(validateEmail $1); then
    ...
    error "The value used for VKPR_ENV_CERT_MANAGER_EMAIL \"$VKPR_ENV_CERT_MANAGER_EMAIL\" is invalid: VKPR_ENV_CERT_MANAGER_EMAIL must consist of lowercase, alphanumeric characters and '.', (e.g. 'default@vkpr.com', regex used for validation is ^[a-z0-9.]+@[a-z0-9]+\.[a-z]+(\.[a-z]+)?$."
    exit
  fi
}
