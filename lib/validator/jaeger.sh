...
validateJaegerDomain (){
    if  $(validateDomain $1); then
        ...
        error "The value used for JAEGER_DOMAIN \"$1\" is invalid: the JAEGER_DOMAIN must consist of a lower case alphanumeric  characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example-vkpr.com', regex used for validation is '^([a-zA-Z0-9][a-zA-Z0-9-]{0,61}[a-zA-Z0-9].)+([a-zA-Z]{2,})|localhost$')"
        exit
    fi
}
