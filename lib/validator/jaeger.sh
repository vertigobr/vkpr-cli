...
validateJaegerIngressClassName (){
    if [[ "$1" =~ ^([a-z]+)$ ]]; then
       return
        ...
        error "The value used for VKPR_ENV_JAEGER_INGRESS_CLASS_NAME \"$VKPR_ENV_JAEGER_INGRESS_CLASS_NAME\" is invalid: VKPR_ENV_JAEGER_INGRESS_CLASS_NAME must consist of lowercase alphanumeric characters, (e.g. 'nginx', regex used for validation is ^([a-z]+)$')"
        exit
    fi
}
...
validateJaegerSslCrtPath (){
    if $(validatePath $1); then
        ...
        error "The value used for VKPR_ENV_JAEGER_CERTIFICATE \"$VKPR_ENV_JAEGER_CERTIFICATE\" is invalid: VKPR_ENV_JAEGER_CERTIFICATE must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. '/tmp/certificate.crt', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
        exit
    fi
}
...
validateJaegerSslKeyPath (){
    if $(validatePath $1); then
        ...
        error "The value used for VKPR_ENV_JAEGER_KEY \"$VKPR_ENV_JAEGER_KEY\" is invalid: VKPR_ENV_JAEGER_KEY must consist of lowercase, uppercase or '-' alphanumeric characters, (e.g. '/tmp/certificate.key', regex used for validation is ^(\/[^\/]+){1,}\/?$')"
        exit
    fi
}
